...
SELECT Customer_ID AS Id_producto,
       DATE_FORMAT(STR_TO_DATE(Fecha, '%m/%d/%Y'), '%m/%d/%Y') AS Fecha,
       Precio
FROM (
    SELECT *,
           ROW_NUMBER() OVER (PARTITION BY Customer_ID ORDER BY STR_TO_DATE(Fecha, '%m/%d/%Y') DESC) AS rn
    FROM Brandon.PD
) t
WHERE rn = 1;
